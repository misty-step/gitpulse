...
- [ ] **Replace terminal green theme with high-contrast modern colors** - In `tailwind.config.ts`, update the color palette: primary green `#00ff00` → `#10B981` (emerald-500), background `#0a0e0a` → `#ffffff`, text `#00ff00` → `#111827` (gray-900). The Matrix aesthetic is fun but reduces readability by ~40%.
...
- [ ] **Merge all filter components into single horizontal toolbar** - Create new `src/components/dashboard/CompactToolbar.tsx` combining ActivityMode + DateRange + Generate button in single 48px height bar. Replace the three separate sections in dashboard page.
...
- [ ] **Add optimistic UI updates** - When Generate is clicked, immediately show skeleton loader for results section while API calls happen. Don't make UI feel frozen during 2-3 second generation time.
