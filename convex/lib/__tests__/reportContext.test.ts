...
describe("buildReportContext", () => {
  ...
  it("includes every event in the timeline and surfaces canonicalText", () => {
    ...
    expect(timeline[2]?.canonicalText).toBe("Older event canonical");
  });

  it("keeps all events even when more than 200 are supplied", () => {
    const events = Array.from({ length: 220 }, (_, idx) =>
      createEvent({
    ...
    const { timeline, context } = buildReportContext({
      events,
      reposById: new Map([[events[0]!.repoId, createRepo()]]),
      startDate: events[0]!.ts - 1000,
      endDate: events[events.length - 1]!.ts + 1000,
    });

    expect(timeline).toHaveLength(events.length);
    expect(context.totals.eventCount).toBe(events.length);
